...
class Twitsense_Twitter_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !$tweets = wp_cache_get( 'widget-twitter-' . $this->number , 'widget' ) ) {
			...
			$twitter_json_url = esc_url_raw( $public_tweets, array('http', 'https') );

            $cached_json_url = esc_url_raw( $cached_tweets, array('http', 'https') );

            // echo "<p>{$twitter_json_url}</p>";
		...
		if ( 'error' != $tweets ) :
			...
			foreach ( (array) $tweets as $tweet ) {
				...
				if ( isset($tweet['user']['screen_name']) )
					$account = urlencode($tweet['user']['screen_name']);
                else
                    $account = "dandrewsify";
